...
function memoCanSearch(target, wordlist, memo = {}) {
    ...
    for (let word of wordlist) {
        if (target.indexOf(word) === 0) {
            const suffix = target.slice(word.length);
            if (memoCanSearch(suffix, wordlist, memo)) {
                memo[target] = true;
                return true;
            }
        }
...
test_1 = memoCanSearch('abcdefgh', ['ab', 'gh', 'abc', 'cd', 'h', 'def']);
test_2 = memoCanSearch('bananna', ['an', 'n', 'ban', 'a', 'b']);
test_3 = memoCanSearch('test', ['es', 'tt', 'st', 'tes']);
